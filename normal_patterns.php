<?php
	...
	$normal_patterns = array(
       ...
       // "(\\b($C"."[i]($C))([o]$C))" ,// optional need check this pattern.
        //"(\\b($C$V($C))([e]$C))",
        "/(\\b({$C}[i]($C))([e]$C))/u", //6
        "/(\\b({$C}[i]($C))([a]{$C}[o][n]))/ui", //7
        //"/(\\b([h]|[m]|[ˀ]|[n]|[t]|[y])[a]($C))([i]$C)/u", // yahid pattern change to exception
        "/(\\b([hmˀnty][u]($C))([a]$C)\\b)/ui", //8
        "/(\\b([nythˀ][i]($C))([a]{$C}[e]$C))/u", //9
        "/\\b({$C}[e]([ˀˁr]))([u]$C)$suffix\\b/u", //10
        "/\\b({$C}[i]($C))([i]{$C1}[uai]?)\\b/u", //11
        "/\\b([h][i]($C))([i]$C)\\b/u", //12
        //"/(\\b([nytmˀ]?$C"."[aiu]($C))($V$C)\\b)/u", //13 not sure if needed.
        "/(\\b([h][i][sš][t][a]($C))([e]$C))/u", //14
        "/(\\b([h][i][z][d][a]($C))([e]$C))/u", //15
        "/(\\b([h][i][ç][ṭ][a]($C))([e]$C))/u", //16
        "/(\\b([nytmhˀ][i][t]{$C}[a]($C))($C$suffix1))/u", //17 hityasbut /
        "/\\b([m]{$C}[ua]({$C}))([a]{$C})\\b/u", // myuḥad
        "/\\b({$G})a({$C}i{$C}a)\\b/u", // hariga ->hriga
        "/\\b({$CNOG})e({$C}i{$C}a)\\b/u", // yešiba -> yšiba    
